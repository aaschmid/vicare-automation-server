...
app.add_middleware(RequestTrackingMiddleware, request_tracker=get_request_tracker())


@app.exception_handler(PyViCareRateLimitError)
def rate_limit_exception_handler(_request, exc):
...
def invalid_config_exception_handler(_request, exc):
...
def not_supported_exception_handler(_request, exc):
